...
// const { data: ohlc } = useOHLC(symbol, range);

// Symbol chip (prefetch on hover)
// const prefetch = usePrefetchSymbol();
// <Link href={`/stocks/${sym}`} onMouseEnter={() => prefetch(sym)}>{sym}</Link>
  ...
  return (
    <div className="p-6">
      ...
      >
        ...
        {/* Pass range to your chart if supported */}
      ...
      {/* Paper trade modal (optional, wire later) */}
      {ticketOpen && (
        <Modal
          onClose={() => setTicketOpen(false)}
          title={`Paper trade ${String(symbol).toUpperCase()}`}
        >
          {/* If you created the ticket component earlier, import it and render here */}
          {/* import OrderTicket from '@/app/paper/ticket'; then: */}
          {/* <OrderTicket symbol={String(symbol).toUpperCase()} onSubmit={() => setTicketOpen(false)} /> */}
